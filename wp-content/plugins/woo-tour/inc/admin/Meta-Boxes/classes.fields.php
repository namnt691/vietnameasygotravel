<?php
// This function clone from wp core and disable add_action to fix auto metadata from WPML version 4.3.6
function wt_delete_metadata( $meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false ) {
    global $wpdb;
...
abstract class EXC_MB_Field {
	...
	public function save( $post_id, $values ) {
		...
		if($this->args['repeatable'] || empty($this->values)){
			if(class_exists('SitePress') && ($this->id == 'wt_accom_service' || $this->id == 'wt_custom_metadata')){
				wt_delete_metadata( 'post',$post_id, $this->id );
			}else{
...
class EXC_MB_Date_Timestamp_Field extends EXC_MB_Field {
	...
	public function html() {
		$dfm = 'm\/d\/Y';
		$tfm = 'h:i A';
		$stdfm = get_option('wt_date_picker');
		if($stdfm=='dmy'){
			$dfm = 'd\/m\/Y';
			$tfm = 'H:i';
		} ?>
...
class EXC_MB_Datetime_Timestamp_Field extends EXC_MB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'exc_mb-timepicker', trailingslashit( EXC_MB_URL ) . 'js/jquery.timePicker.min.js', array( 'jquery', 'exc_mb-scripts' ) );
		wp_enqueue_script( 'exc_mb-datetime', trailingslashit( EXC_MB_URL ) . 'js/field.datetime.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', 'exc_mb-scripts' ),'1.1' );
	}
	...
	public function html() { 
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_small exc_mb_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo $this->value ? esc_attr( date( $dfm, $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'exc_mb_text_small exc_mb_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo $this->value ? esc_attr( date( $tfm, $this->value ) ) : '' ?>" />
