...
class CMB2 extends CMB2_Base {
	...
	public function render_group_row( $field_group ) {
		...
		echo '
		<div class="postbox cmb-row cmb-repeatable-grouping', esc_attr($closed_class), '" data-iterator="',esc_attr( $field_group->index), '">';

		if ( $field_group->args( 'repeatable' ) ) {
			echo '<button type="button" data-selector="', esc_attr($field_group->id()), '_repeat" class="dashicons-before dashicons-no-alt cmb-remove-group-row" title="', esc_attr( $field_group->options( 'remove_button' ) ), '"></button>';
		}
