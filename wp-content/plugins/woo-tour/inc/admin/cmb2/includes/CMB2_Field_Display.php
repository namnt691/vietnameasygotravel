	...
	public function display() {
		...
		if ( $this->field->args( 'repeatable' ) ) {
			...
			if ( is_array( $this->field->value ) ) {
				...
				echo '<ul class="cmb2-' . esc_attr(str_replace( '_', '-', $this->field->type() )) . '">';
				foreach ( $this->field->value as $val ) {
					$this->value = $val;
					echo '<li>', wp_kses_post($this->_display()), '</li>';
					;
				}
				echo '</ul>';
			}
...
class CMB2_Display_Checkbox extends CMB2_Field_Display {
	 ...
	 */
	protected function _display() {
		echo esc_attr($this->value === 'on' ? 'on' : 'off');
	}
}
...
class CMB2_Display_Select extends CMB2_Field_Display {
	...
	protected function _display() {
		...
		if ( ! $this->value && $fallback ) {
			...
			echo wp_kses_post($options[ $this->value ]);
		} else {
...
class CMB2_Display_Text_Time extends CMB2_Field_Display {
	 ...
	 */
	protected function _display() {
		echo wp_kses_post($this->field->get_timestamp_format( 'time_format', $this->value ));
	}
}
...
class CMB2_Display_Text_Date extends CMB2_Field_Display {
	 ...
	 */
	protected function _display() {
		echo wp_kses_post($this->field->get_timestamp_format( 'date_format', $this->value ));
	}
}
...
class CMB2_Display_Text_Date_Timezone extends CMB2_Field_Display {
	...
	protected function _display() {
		...
		echo wp_kses_post($date, ( $time ? ' ' . $time : '' ), ( $tzstring ? ', ' . $tzstring : '' ));
	}
}
	...
	protected function file_output( $url_value, $id, CMB2_Type_File_Base $field_type ) {
		...
		if ( $field_type->is_valid_img_ext( $url_value ) ) {
			...
			if ( $id ) {
				...
				$image = '<img class="cmb-image-display" style="max-width: ' . absint( $size ) . 'px;" src="' . $url_value . '" alt="" />';
			}
