<?php
function parse_wt_list_func($atts, $content){
	if(is_admin()){ return;}
	...
	if($the_query->have_posts()){?>
		<div class="wt-grid-shortcode wt-tourlist wt-grid-column-1" id="grid-<?php echo $ID;?>">
        	<div class="ct-grid">
				...
				if($posts_per_page<$count){
					...
					echo '
						<div class="ex-loadmore">
							...
							<a  href="javascript:void(0)" class="loadmore-grid lm-list" data-id="grid-'.$ID.'">
...
add_shortcode( 'wt_list', 'parse_wt_list_func' );
add_action( 'after_setup_theme', 'wt_reg_list_vc' );
function wt_reg_list_vc(){
	if(function_exists('vc_map')){
	vc_map( array(
	   "name" => esc_html__("WooTours - List", "woo-tour"),
	   "base" => "wt_list",
	   "class" => "",
	   "icon" => "icon-list",
	   "controls" => "full",
	   ...
	   "params" => array(
		  ...
		  array(
		  	"admin_label" => true,
			"type" => "textfield",
			"heading" => esc_html__("Location", "woo-tour"),
			"param_name" => "location",
			"value" => "",
			"description" => esc_html__("List of locations, separated by a comma", "woo-tour"),
		  ),
		  array(
		  	"admin_label" => true,
			 "type" => "dropdown",
		  ...
		  array(
		  	"admin_label" => true,
			"type" => "textfield",
			"heading" => esc_html__("Phone", "woo-tour"),
			"param_name" => "phone",
			"value" => "",
			"description" => esc_html__("Enter Phone number", "woo-tour"),
		  ),
		  array(
		  	"admin_label" => true,
			"type" => "textfield",
			"heading" => esc_html__("Quotes link", "woo-tour"),
			"param_name" => "quotes_link",
			"value" => "",
			"description" => esc_html__("Enter get Quotes link", "woo-tour"),
		  ),
		  /*array(
		  	"admin_label" => true,
			"type" => "textfield",
